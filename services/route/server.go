...
import (
	...
	"github.com/patil-kshitij/hotrod/pkg/delay"
	"github.com/patil-kshitij/hotrod/pkg/httperr"
	"github.com/patil-kshitij/hotrod/pkg/log"
...
func (s *Server) createServeMux() http.Handler {
	r := mux.NewRouter() //tracing.NewServeMux(s.tracer)
	r.Use(otelmux.Middleware("HOT-R.O.D. Server"))
	r.Handle("/route", http.HandlerFunc(s.route))
	r.Handle("/debug/vars", expvar.Handler()) // expvar
	r.Handle("/metrics", promhttp.Handler())  // Prometheus
	return r
}
...
func computeRoute(ctx context.Context, pickup, dropoff string) *Route {
	...
	_, span := tracer.Start(ctx, "computeRoute", trace.WithSpanKind(trace.SpanKindInternal))
	defer span.End()
